...
(defn- init-xs-hdlrs
  ...
  (mc/add-handler! device play frame-key (fn [_val _val-f] (instr/handle-event (+ (ot/now) 500) [:play instr])))
  (mc/add-handler! device stop frame-key (fn [_val _val-f] (instr/handle-event (+ (ot/now) 500) [:stop instr])))
